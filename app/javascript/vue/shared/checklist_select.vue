<template>
  <div class="w-full relative" ref="container" v-click-outside="{ handler: 'close', exclude: [] }">
    ...
    <perfect-scrollbar
      ...
      class="relative scroll-container p-2.5 pt-0 bg-white rounded border-[1px] border-sn-light-grey shadow-flyout-shadow max-h-[25rem]"
      :class="{
        'block': isOpen,
        'hidden': !isOpen
      }"
    >
      <div v-if="withButtons" class="sticky z-10 top-0 bg-white">
        <div class="pb-1 pt-2.5 rounded flex gap-1 justify-start items-center">
          ...
          <div class="btn btn-light"
               ...
               :class="{
                 'disabled': options.length === selectedValues.length
               }"
              >{{ i18n.t('general.select_all') }}</div>
        </div>
      </div>
      <div v-if="options.length" class="flex flex-col gap-[1px]">
        <div
          ...
          :key="option.id"
          class="px-2 py-1 rounded hover:bg-sn-super-light-grey flex gap-1 justify-start"
        >
